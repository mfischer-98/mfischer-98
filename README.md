<img src="https://readme-typing-svg.demolab.com/?font=Fira+Code&weight=600&size=28&pause=1200&color=FFFFFF&vCenter=true&width=420&lines=Hey+there%2C+I%27m+Melissa+%E2%9C%A8" alt="Typing SVG" />

<p>
  <img src="https://img.shields.io/badge/Software_Engineer-f5c2e7?style=for-the-badge" />
  <a href="https://www.42porto.com/pt/" target="_blank"><img src="https://img.shields.io/badge/42_Porto-000000?style=for-the-badge&logo=42&logoColor=white" /></a>
  <img src="https://img.shields.io/badge/Porto%2C_Portugal-f8dda2?style=for-the-badge" />
</p>

I’m a software engineer building projects at **42 Porto** with a focus on **C/Linux**, graphics programming, embedded systems and game development. My path into tech comes from a mix of biology/science, animation and programming, so I like building logical systems and solving problems creatively.

I am currently working as an Animation Workshop Leader at TUMO Porto and on my **C++** projects at 42 Porto.

## Stack
<p>
  <img src="https://img.shields.io/badge/C-e9d5ff?style=for-the-badge&logo=c&logoColor=1e1e2e" />
  <img src="https://img.shields.io/badge/Linux-f8dda2?style=for-the-badge&logo=linux&logoColor=1e1e2e" />
  <img src="https://img.shields.io/badge/Bash-9ADBB3?style=for-the-badge&logo=gnubash&logoColor=1e1e2e" />
  <img src="https://img.shields.io/badge/Git-f5c2e7?style=for-the-badge&logo=git&logoColor=1e1e2e" />
  <img src="https://img.shields.io/badge/GitHub-1e1e2e?style=for-the-badge&logo=github&logoColor=ffffff" />
  <img src="https://img.shields.io/badge/VS_Code-dbc7ff?style=for-the-badge&logo=visualstudiocode&logoColor=1e1e2e" />
  <img src="https://img.shields.io/badge/Unity-ffd9e8?style=for-the-badge&logo=unity&logoColor=1e1e2e" />
</p>

## 42 Projects

| Project | Description | Status | Link |
|---------|-------------|--------|------|
| netpractice | A networking project for understanding IP addressing, routing and basic network configuration. | In progress | [Repository](https://github.com/mfischer-98/NetPractice) |
| CPP00-04 | C++ exercises to learn the language and object-oriented programming. | In progress | [Repository](https://github.com/mfischer-98/CPPs) |
| cub3D | A 3D engine project focused on raycasting, rendering and spatial logic in C. | Completed | [Repository](https://github.com/mfischer-98/cub3D) |
| minishell | A shell built in C with parsing, pipes, redirections, processes and signal handling. | Completed | [Repository](https://github.com/mfischer-98/Minishell) |
| philosophers | A concurrency project about threads, mutexes and shared concurrency programming. | Completed | [Repository](https://github.com/mfischer-98/Philosophers) |
| so_long | A top-down 2D graphical game focused on event handling, map validation and game loop basics. | Completed | [Repository](https://github.com/mfischer-98/So_long) |
| push_swap | A sorting project centered on algorithmic thinking, optimization and constrained operations. | Completed | [Repository](https://github.com/mfischer-98/Push_swap) |
| minitalk | A client-server communication project using UNIX signals. | Completed | [Repository](https://github.com/mfischer-98/Minitalk) |
| ft_printf | A custom implementation of printf to practice variadic functions and formatted output. | Completed | [Repository](https://github.com/mfischer-98/ft_printf) |
| get_next_line | A file-reading project focused on buffers, memory handling and static variables. | Completed | [Repository](https://github.com/mfischer-98/Get-Next-Line) |
| Libft | A foundational C library with custom implementations of common standard library functions. | Completed | [Repository](https://github.com/mfischer-98/Libft) |

## Game Projects

| Game | Type | Description | Link |
|------|-----|-------------|------|
| Elixir of the End | GameJam | Find out what is happening in the village, gather ingredients and salve your people. | [Repository](https://github.com/DadiaZ24/Elixir-of-the-End) |
| Final Feather | Club Project | You are a little sparrow that was summoned by a wizard, now you have to survive the attacks of his monsters. | [Repository](https://github.com/DadiaZ24/The-Final-Feather-game-) |
| Lost in Space | GameJam | Travel in space, visit planets and choose wisely to increase your chances of surviving. | Link available soon |

## Contact

Feel free to reach out below! I’d love to connect and chat about projects, collaborations, and opportunities.

<p><a href="https://www.linkedin.com/in/-melissa-fischer" target="_blank"><img src="https://img.shields.io/badge/LinkedIn-Melissa_Fischer-1e1e2e?style=for-the-badge&logo=linkedin&logoColor=ffffff" /></a><a href="mailto:melissafisc98@gmail.com"><img src="https://img.shields.io/badge/EMAIL-f9c6d3?style=for-the-badge&logo=gmail&logoColor=1e1e2e" /></a></p>
